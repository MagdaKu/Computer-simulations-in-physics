...
def compute_energies_and_pressure(particles, particle_number, box_size, eps, sigma, kB):
    ...
    for i, p1 in enumerate(particles):
        for j in range(i + 1, particle_number):  #Only iterating over each pair once
            p2 = particles[j]
            distance = closest_image(p1.r, p2.r, box_size)
            r = np.linalg.norm(distance)
            if r < 1e-12 or r > 2.5 * sigma: #LJ potential cut-off
                continue
...
plt.plot(os_x, total_energy)
plt.subplot(3,1,2)
plt.title("Temperature")
plt.plot(os_x, temperature_list)
plt.subplot(3,1,3)
plt.title("Pressure")
plt.plot(os_x, pressure_list)
plt.tight_layout()
plt.show()
...
with imageio.get_writer(file_path + 'animation.gif', mode='I', duration = 1) as writer:
